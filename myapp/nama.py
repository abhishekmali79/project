...
from logic import calculate,safe_int
import pandas as pd
...
def on_item_changed(item):
    ...
    if item.column()==2:
        ...
        result=calculate(table.item(item.row(),0).text(),table.item(item.row(),1).text(),item.text(),table.item(item.row(),6).text())

        safe_set(item.row(), 3, result["Tdebt"])
        safe_set(item.row(), 2, result["text"])
        safe_set(item.row(), 4, result["total"])
        safe_set(item.row(), 5, result["raw_text"])
        safe_set(item.row(), 6, result["paid"])
        safe_set(item.row(), 7, result["remaining"])
    ...
    if item.column()==1:
        ...
        result=calculate(table.item(item.row(),0).text(),item.text(),table.item(item.row(),2).text(),table.item(item.row(),6).text())
...
# If TOTAL is empty (at start of day), TOTAL = DEBT + Tdebt.
        old_total = safe_int(result["total"] if result["total"] else 0)
        ...
        paid = safe_int(result["paid"] if result["paid"] else 0)
    ...
    if item.column() == 6:  # PAID changed
        ...
        result=calculate(table.item(item.row(),0).text(),table.item(item.row(),1).text(),table.item(item.row(),2).text(),item.text())
...
def set_table():
    ...
    table.setItem(row, 1, QTableWidgetItem(str(result["debt"])))
    table.setItem(row, 2, QTableWidgetItem(result["text"]))
    table.setItem(row, 3, QTableWidgetItem(str(result["Tdebt"])))
    table.setItem(row, 4, QTableWidgetItem(str(result["total"])))
    table.setItem(row, 5, QTableWidgetItem(result["raw_text"]))
    table.setItem(row, 6, QTableWidgetItem(str(result["paid"])))
    table.setItem(row, 7, QTableWidgetItem(str(result["remaining"])))
